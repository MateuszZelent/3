...
func init() {
	...
	DeclFunc("Layers", Layers, "Part of space between cell layer1 (inclusive) and layer2 (exclusive), in integer indices. Note: based on the current cell size, so may no longer be valid if cell size is changed.")
	DeclFunc("Layer", Layer, "Single layer (along z), by integer index starting from 0. Note: based on the current cell size, so may no longer be valid if cell size is changed.")
