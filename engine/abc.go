...
// effectiveAlphaMSlice materializes Alpha+SpongeAlpha when an absorbing
// boundary is active. Keeping this combination outside the torque kernels
// preserves upstream's complete PTX architecture set.
func effectiveAlphaMSlice() cuda.MSlice {
	if SpongeAlpha.isZero() {
		return Alpha.MSlice()
	}
	alpha, recycleAlpha := Alpha.Slice()
	if recycleAlpha {
		defer cuda.Recycle(alpha)
	}
	sponge, recycleSponge := SpongeAlpha.Slice()
	if recycleSponge {
		defer cuda.Recycle(sponge)
	}
	combined := cuda.Buffer(1, Mesh().Size())
	cuda.Madd2(combined, alpha, sponge, 1, 1)
	return cuda.ToMSlice(combined)
}
...
func AutoAbsorbingBoundary(maxFreqGHz float64, direction string, maxAlpha float64, nWavelengths float64) {
	ms := Msat.getRegion(0)[0]
	aex := Aex.getRegion(0)[0]
	alpha0 := Alpha.getRegion(0)[0]
	if ms == 0 {
	...
	bExt := B_ext.perRegion.GetRegion(0)
	bMag := math.Sqrt(bExt[0]*bExt[0] + bExt[1]*bExt[1] + bExt[2]*bExt[2])
	hExt := bMag / mu0ABC
	d := float64(Mesh().Size()[Z]) * Mesh().CellSize()[Z]
	omegaMax := 2 * math.Pi * maxFreqGHz * 1e9
	cellX := Mesh().CellSize()[X]
	cellY := Mesh().CellSize()[Y]
...
func autoAbsorbingBoundaryAdvanced(
	...
	if spec.XMinus {
		res := designAutoABCSide(Mesh().WorldSize()[X], Mesh().CellSize()[X], d, ms, aex, heffEstimate.Used, fMinGHz, fMaxGHz, mode, maxAlpha, targetRLdB, targetEdgeAmpdB, xThetaDeg, dmiJm2, -1, spec.XMinus && spec.XPlus)
		if !res.Valid {
	...
	if spec.YMinus {
		res := designAutoABCSide(Mesh().WorldSize()[Y], Mesh().CellSize()[Y], d, ms, aex, heffEstimate.Used, fMinGHz, fMaxGHz, mode, maxAlpha, targetRLdB, targetEdgeAmpdB, yThetaDeg, dmiJm2, -1, spec.YMinus && spec.YPlus)
		if !res.Valid {
	...
	log.Log.Info("│ Material: Msat = %.3e A/m, Aex = %.3e J/m, α = %.4f", ms, aex, alpha0)
	log.Log.Info("│ |B_ext| = %.4f T  → H_ext = %.3e A/m", heffEstimate.B_extMag, heffEstimate.HExt)
...
func baseAutoABCHeffEstimate(ms, ku1Jm3 float64) abcHeffEstimate {
	...
	bExt := B_ext.perRegion.GetRegion(0)
	estimate.B_extMag = math.Sqrt(bExt[0]*bExt[0] + bExt[1]*bExt[1] + bExt[2]*bExt[2])
	estimate.HExt = estimate.B_extMag / mu0ABC
	if ku1Jm3 != 0 {
...
func canUseProjectedAutoABCHeff() (bool, string) {
	if !relaxing {
		tempR0 := Temp.getRegion(0)[0]
		if math.Abs(tempR0) > 0 {
			return false, "projected H_eff was skipped because region-0 temperature is non-zero while not relaxing, so B_eff may include B_therm"
		}
	}
	return true, ""
}
...
func estimateAutoABCHeff(ms, ku1Jm3 float64) abcHeffEstimate {
	...
	bEffAvg := B_eff.Region(0).Average()
