...
func FftTrack(q Quantity, minFreqGHz, maxFreqGHz, dFreqGHz float64) {
	...
	if globalFft.dFreq <= 0 {
		panic(UserErr("FftTrack: dFreqGHz must be > 0"))
	}
	if globalFft.maxFreq <= globalFft.minFreq {
		panic(UserErr("FftTrack: maxFreqGHz must be > minFreqGHz"))
	}
	...
	fftLogf("┌─ FFT tracking configured ─────────────────")
	fftLogf("│ Quantity:    %s (%d components)", name, nComp)
	fftLogf("│ Freq range:  %.2f – %.2f GHz", minFreqGHz, maxFreqGHz)
	fftLogf("│ Resolution:  %.3f GHz (%d bins)", dFreqGHz, globalFft.nFreqs)
	fftLogf("│ Sample every: %.3f ns (Nyquist for %.2f GHz)", globalFft.sampleInterval*1e9, maxFreqGHz)
	fftLogf("│ Batch size:  %d steps", globalFft.stepsPerBatch)
	fftLogf("│ Spectrogram segment: %.2f ns", globalFft.segDuration*1e9)
	fftLogf("└────────────────────────────────────────────")
}
...
func SetFftMaxFreqGHz(maxFreqGHz float64) error {
	...
	globalFft.sampleInterval = 1 / (2 * globalFft.maxFreq)
	resetFftDataLocked()
	fftLogf("FFT max frequency updated to %.2f GHz, sampling every %.3f ns; FFT data reset", maxFreqGHz, globalFft.sampleInterval*1e9)
	return nil
}
...
func FftSave() {
	...
	if spectrum == nil || freqAxis == nil {
		fftLogf("FftSave: no FFT data to save")
		return
	}
	...
	if err != nil {
		fftErrf("FftSave: %v", err)
		return
	}
	defer func() {
		if err := f.Close(); err != nil {
			fftErrf("FftSave close: %v", err)
		}
	}()
	...
	fftLogf("FFT spectrum saved to %s", fname)
}
...
func FftSavePerCell() {
	fftLogf("FftSavePerCell: For per-cell FFT, save m snapshots with AutoSaveAs(m, \"m_timeseries\", period)")
	fftLogf("Then post-process: fft = np.fft.fft(m_data, axis=0) along the time axis.")
}
